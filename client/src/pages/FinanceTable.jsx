...
import { getCashSummary, getAccounts, getSaleDropdown, getPurchaseDropdown, createFinanceTransaction } from '../services/financeService'
...
export default function FinanceTable() {
  ...
  const { register, handleSubmit, reset, formState: { errors }, watch } = useForm({
    ...
    defaultValues: { account: '', type: 'inflow', amount: '', description: '', notes: '', linked_sale: '', linked_purchase: '' }
  })
  ...
  const { data: metadata } = useQuery({
    ...
    queryFn: async () => {
      const [accountsRes, salesRes, purchasesRes] = await Promise.all([
        getAccounts(), getSaleDropdown(), getPurchaseDropdown()
      ])
      return {
  ...
  function openModal() {
    reset({ account: '', type: 'inflow', amount: '', description: '', notes: '', linked_sale: '', linked_purchase: '' })
    setModalOpen(true)
  }
          ...
          <div className="bg-white border text-sm md:text-base border-gray-200 rounded-2xl shadow-sm mb-8 overflow-hidden">
            <div className="p-4 md:p-5 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-50/30">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-1 md:justify-end">
                ...
                <div className="relative">
                  ...
                  {filterOpen && (
                    <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-xl z-30 overflow-hidden">
                      <div className="py-2">
                        {[
                          { k: 'all', l: 'All Types' },
                          { k: 'inflow', l: 'Inflow' },
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Description</th>
                  <th scope="col" className="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Type</th>
                  <th scope="col" className="px-6 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Amount</th>
                  <th scope="col" className="px-6 py-3 text-right text-sm font-semibold text-gray-700 uppercase tracking-wider">Balance</th>
                </tr>
              </thead>
                ...
                {loadingSummary ? (
                  ...
                  transactions.map(tx => (
                    <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-6 border-b border-gray-100">
                        <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-full uppercase tracking-wider", tx.type === 'inflow' ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>
                          {tx.type}
                        </span>
                      </td>
                      <td className="py-4 px-6 border-b border-gray-100">
                        <p className={cn("font-bold", tx.type === 'inflow' ? "text-green-600" : "text-red-600")}>
                          {formatMoney(tx.amount, tx.type === 'inflow' ? '+' : '-')}
                        </p>
                      </td>
