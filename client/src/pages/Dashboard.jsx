...
import { LayoutDashboard, ShoppingCart, LogOut, ArrowRightLeft } from 'lucide-react'
...
export default function Dashboard() {
  ...
  // PHASE 11: Replace 14 manual Promise.all endpoints with a unified cached React Query fetching pipeline
  // Data automatically remains cached inside the app routing, saving loading time!
  ...
  // PHASE 12: Adopt the React Query Mutator pattern to handle fast Form Submissions
  // with cached UI invalidation (refreshing data auto)
  const submitMutator = useMutation({
    mutationFn: async ({ type, payload }) => {
      if (type === 'buy') return createPurchase(payload)
      if (type === 'sell') return createSale(payload)
      if (type === 'expense') return createExpense(payload)
              ...
              <AnimatePresence>
                {isModalOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed inset-0 bg-black/50 flex flex-col items-center justify-center z-50 p-4 backdrop-blur-sm"
                  >
                    <motion.div
                      // Smooth Modal Expansion Pop-in
                    ...
                    >
                      <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-2xl">
                        <h2 className="text-xl font-bold tracking-tight text-gray-900">Add New Data</h2>
                        <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 transition-colors bg-white w-8 h-8 rounded-full border shadow-sm flex items-center justify-center hover:bg-gray-50">
                        ...
                        {!loadingDropdowns && (
                          <div className="mb-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                              ...
                              >
                                <div className={`w-10 h-10 mx-auto mb-2 flex items-center justify-center rounded-lg transition-colors ${selectedType === 'sell' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'}`}>
                              ...
                              <button
                                  ...
                                  className={`!rounded-button whitespace-nowrap p-4 border-2 rounded-xl transition-all text-center ${
                                    selectedType === 'buy' ? 'border-primary bg-primary/5 ring-1 ring-primary/30' : 'border-gray-200 hover:border-primary hover:bg-primary/5'
                                  }`}
                              >
                                <div className={`w-10 h-10 mx-auto mb-2 flex items-center justify-center rounded-lg transition-colors ${selectedType === 'buy' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'}`}>
                              ...
                              <button
                                  ...
                                  className={`!rounded-button whitespace-nowrap p-4 border-2 rounded-xl transition-all text-center ${
                                    selectedType === 'expense' ? 'border-primary bg-primary/5 ring-1 ring-primary/30' : 'border-gray-200 hover:border-primary hover:bg-primary/5'
                                  }`}
                              >
                                <div className={`w-10 h-10 mx-auto mb-2 flex items-center justify-center rounded-lg transition-colors ${selectedType === 'expense' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'}`}>
                                  <i className="ri-receipt-line ri-lg"></i>
                                </div>
                      ...
                      <div className="p-6 border-t border-gray-100 bg-gray-50">
                        <div className="flex justify-end gap-3">
                          <button
                              ...
                              className="px-5 py-2.5 !rounded-button whitespace-nowrap border border-gray-300 font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm"
                          >
                            Cancel
                          </button>
                          <button
                              onClick={() => {
                                ...
                                if (selectedType === 'buy') {
                                  ...
                                  payload = {
                                    ...
                                    payment_method: expenseForm.payment_method ? expenseForm.payment_method : null,
                                    account: expenseForm.account ? Number(expenseForm.account) : undefined
                                  }
                                }
