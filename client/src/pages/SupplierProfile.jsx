  ...
  useEffect(() => {
    ...
    async function loadSupplier() {
      try {
        ...
        const data = await svc.getSuppliers()
        const list = Array.isArray(data) ? data : (data?.results ?? [])
        const s = list.find(x => Number(x.id) === Number(id)) || null
        if (mounted) setSupplier(s)
      } catch (err) { if (mounted) setSupplier(null) }
      finally { if (mounted) setLoadingSupplier(false) }
    }
    loadSupplier()
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        setLoading(true)
        const svc = await import('../services/supplierService')
        const txData = await svc.getSupplierTransactions(id, { search: searchQuery, ordering, page: txPage })
        const txList = Array.isArray(txData) ? txData : (txData?.results ?? [])
        if (mounted) {
          setTransactions(txList)
          if (txData && !Array.isArray(txData)) setTxMeta({ count: txData.count, next: txData.next, previous: txData.previous, pageSize: (txData.results || []).length })
          else setTxMeta({ count: txList.length, next: null, previous: null, pageSize: txList.length })
        }
      } catch (err) {
    ...
    function onDoc(e) { if (sortOpen || exportOpen) { setSortOpen(false); setExportOpen(false) } }
    document.addEventListener('click', onDoc)
    return () => { mounted = false; clearTimeout(timer); document.removeEventListener('click', onDoc) }
  }, [id, txPage, searchQuery, ordering])

  async function downloadExport(format) {
    const hasTx = (txMeta && txMeta.count > 0) || (transactions && transactions.length > 0)
    if (!hasTx) { toast.error('No transactions to export'); setExportOpen(false); return }
    try {
      toast.info(`Exporting ${format.toUpperCase()}...`)
      const filename = await supplierService.exportSupplierTransactions(id, format)
