...
import {
  ...
  Plus, Package, X, Anchor
} from 'lucide-react'
import {
  getStockMovements, getStockMovementDetail,
  createStockMovement,
  getInventoryDropdown, getPurchaseDropdown, getSaleDropdown
...
const movementSchema = z.object({
  ...
  quantity: z.number().min(1, 'Quantity must be at least 1')
      .or(z.number().max(-1, 'Quantity cannot be zero')),
...
export default function InventoryStockMovements() {
  ...
  const { data: inventoryOptionsRaw } = useQuery({
    ...
    queryFn: () => getInventoryDropdown({ search: debouncedInventorySearch }),
    staleTime: 30 * 1000
  })
  // Service already unwraps — handle both array and paginated shapes
  const inventoryOptions = useMemo(() => {
    if (Array.isArray(inventoryOptionsRaw)) return inventoryOptionsRaw
    if (Array.isArray(inventoryOptionsRaw?.results)) return inventoryOptionsRaw.results
    return []
  }, [inventoryOptionsRaw])

  // ── Purchase + Sale Dropdowns ────────────────────────────────
  const { data: metadata } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        purchases: Array.isArray(purchases) ? purchases : (purchases?.results ?? []),
        sales: Array.isArray(sales) ? sales : (sales?.results ?? [])
      }
    },
    staleTime: 5 * 60 * 1000
  })

  // ── Core Movements Query ─────────────────────────────────────
  const { data, isLoading: loading } = useQuery({
    queryKey: ['stockMovements', page, debouncedSearch, ordering],
    queryFn: () => getStockMovements({ page, search: debouncedSearch, ordering }),
    placeholderData: (prev) => prev,
    staleTime: 30 * 1000,
  })

  const rawItems = data?.results ?? (Array.isArray(data) ? data : [])
  const count = data?.count ?? 0
  const nextPageUrl = data?.next ?? null
  const prevPageUrl = data?.previous ?? null
  const pageSize = data?.page_size ?? 10

  // ── Detail Query (driven by selectedMovementId) ──────────────
  const { data: detailMovement, isLoading: detailLoading } = useQuery({
    queryKey: ['stockMovementDetail', selectedMovementId],
    queryFn: () => getStockMovementDetail(selectedMovementId),
  ...
  function onSubmit(formData) { createMutator.mutate(formData) }
          ...
          <main className="flex-1 p-6 md:p-8 md:ml-64 w-full transition-all duration-300">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm mt-2">
                Audit all stock interactions including inbound purchases, local adjustments and outbound sales.
              </p>
            </div>

            <div className="mb-6"><InventoryTabs /></div>
              ...
              <div className="p-4 md:p-5 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-50/30">
                ...
                <div className="relative flex items-center gap-3 w-full md:w-auto">
                  <button
                      ...
                      className="w-full md:w-auto px-4 py-2 border border-gray-200 rounded-xl text-sm font-medium flex items-center justify-center gap-2 bg-white text-gray-700 hover:bg-gray-50 shadow-sm"
                  >
                    <ArrowUpDown size={16} /><span>Sort Records</span>
                  </button>
                  {sortOpen && (
                ...
                {loading ? (
                    ...
                    rawItems.map((item) => (
                        <div
                            ...
                            onClick={() => openDetail(item)}
                            className={cn(
                                "bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow cursor-pointer",
                                item.isOptimistic ? "opacity-50" : ""
                            )}
                        >
                      ...
                      <span className="flex items-center gap-1">
                        <Anchor size={12} />{item.warehouse_name}
                      </span>
                            <span className={cn(
                      ...
                      <div className="p-6 md:p-8 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-gray-500 text-sm mt-1">
                            Audit and push a local offset to inventory stock node amounts.
                          </p>
                        </div>
                        <button
                            onClick={closeModal}
                            className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-gray-600 bg-white rounded-full border shadow-sm transition-colors shrink-0"
                        >
                          <X size={20} />
                        </button>
                      </div>
                      <form onSubmit={handleSubmit(onSubmit)} className="p-6 md:p-8 space-y-5">
                        ...
                        <div>
                          ...
                          <select
                              {...register('inventory')}
                              className={cn(
                                  "w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-white",
                                  errors.inventory ? "border-red-500" : "border-gray-200 focus:border-primary"
                              )}
                          >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          <div>
                            ...
                            <select
                                ...
                                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-white focus:border-primary"
                            >
                          ...
                          <div>
                            ...
                            <input
                                ...
                                className={cn(
                                    "w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-gray-50",
                                    errors.quantity ? "border-red-500" : "border-gray-200 focus:border-primary"
                                )}
                                placeholder="+10 or -5"
                            />
                            {errors.quantity && (
                  ...
                  >
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden my-8 border border-gray-100">
                      <div className="p-6 md:p-8 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        ...
                        <button
                            onClick={() => { setDetailOpen(false); setSelectedMovementId(null) }}
                            className="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-gray-600 bg-white rounded-full border shadow-sm transition-colors shrink-0"
                        >
                          <X size={20} />
                        </button>
                      ...
                      <div className="p-6 md:p-8">
                        {detailLoading ? (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-3 justify-between">
                                  <div>
                                    ...
                                    <h4 className="text-lg font-bold text-gray-900 leading-tight">
                                      {detailMovement.inventory?.item}
                                    </h4>
                                  </div>
                                  <div className={cn(
                                      "px-4 py-2 rounded-xl border text-center",
                                      detailMovement.movement_type === 'purchase'
                                          ? 'border-green-200 bg-green-50'
                                          : detailMovement.movement_type === 'sale'
                                              ? 'border-red-200 bg-red-50'
                                              : 'border-blue-200 bg-blue-50'
                                  )}>
                                    <p className={cn(
                                        "text-2xl font-black",
                                        detailMovement.quantity > 0 ? "text-green-700" : "text-red-700"
                                    )}>
                                      {detailMovement.quantity > 0 ? '+' : ''}{detailMovement.quantity}
                                    </p>
                                    <p className={cn(
                                        "text-xs font-bold uppercase tracking-wider",
                                        detailMovement.quantity > 0 ? "text-green-600" : "text-red-600"
                                    )}>
                              ...
                              {detailMovement.inventory?.warehouse}
                            </span>
                                </div>
                                <div>
                                  <p className="text-sm font-semibold text-gray-400 uppercase tracking-wider">
                              ...
                              {formatDate(detailMovement.date)}
                                ...
                                <div>
                                  ...
                                  <span className="text-sm font-medium text-gray-900">
                              {detailMovement.inventory?.current_stock} count
                            </span>
                                </div>
                                <div>
                                  <p className="text-sm font-semibold text-gray-400 uppercase tracking-wider">
                              ...
                              {detailMovement.purchase
                                  ? detailMovement.purchase.reference
                                  : detailMovement.sale
                                      ? detailMovement.sale.reference
                                      : '-'
                              }
                            </span>
                                </div>
