...
import {
  ...
  Edit2, Trash2, Plus, Box, Hash, DollarSign, Filter
} from 'lucide-react'
import {
  getItems, createItem, updateItem, deleteItem, getCategories
...
export default function InventoryItems() {
  ...
  useEffect(() => { setPage(1) }, [debouncedSearch, ordering, categoryFilter])

  useEffect(() => {
    function onDoc() { setSortOpen(false); setFilterOpen(false) }
  ...
  // ── Categories — shared query key with InventoryCategories ──
  const { data: categoriesRaw } = useQuery({
    queryKey: ['inventoryCategories', 1, '', 'name'],
  ...
  const { data, isLoading: loading } = useQuery({
    ...
    queryFn: () => {
      const params = { page, search: debouncedSearch, ordering }
      if (categoryFilter) params.category__name = categoryFilter
  ...
  const saveMutator = useMutation({
    mutationFn: (payload) => {
      ...
      return editingItem ? updateItem(editingItem.id, body) : createItem(body)
    },
    onSuccess: () => {
  ...
  const deleteMutator = useMutation({
    ...
    onSuccess: () => { toast.success('Item deleted successfully'); invalidate() },
    onError: () => toast.error('Failed to delete item'),
  })

  function openEdit(item) { setEditingItem(item); setIsModalOpen(true) }
  ...
  function closeModal() { setIsModalOpen(false); setEditingItem(null); reset() }
  function onSubmit(formData) { saveMutator.mutate(formData) }
          ...
          <main className="flex-1 p-6 md:p-8 md:ml-64 w-full transition-all duration-300">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm mt-2">
                Manage individual product units and track core pricing metrics.
              </p>
            </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-1">
                  ...
                  <div className="relative">
                    <button
                        ...
                        className={cn(
                            "w-full md:w-auto px-4 py-2 border rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-sm whitespace-nowrap",
                            categoryFilter
                          ...
                          <div className="py-2 max-h-64 overflow-y-auto">
                            ...
                            {categories.map(opt => (
                                <button
                                    ...
                                    onClick={(ev) => {
                                      ev.stopPropagation()
                                      setCategoryFilter(opt.name)
                                      setFilterOpen(false)
                                    }}
                                    className={cn(
                                        "w-full text-left px-4 py-2 text-sm transition-colors",
                                        categoryFilter === opt.name
                                            ? "bg-primary/5 text-primary font-medium"
                                            : "text-gray-700 hover:bg-gray-50"
                                    )}
                                >
                ...
                {loading ? (
                    ...
                    rawItems.map((item) => (
                        <div
                            key={item.id}
                            className={cn(
                                "bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow",
                                item.isOptimistic ? "opacity-50" : ""
                            )}
                        >
                      ...
                      <span className="flex items-center gap-1 bg-gray-100 px-2 py-0.5 rounded text-gray-600">
                        <Hash size={12} />{item.code}
                            ...
                            <span className="bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full font-sans tracking-normal font-medium">
                        {categoryLabel(item.category)}
                      </span>
                        ...
                        /{item.unit_measure || 'unt'}
                          ...
                          <div className="flex items-center gap-2 pt-3 border-t border-gray-50 justify-end">
                            <button
                                onClick={(ev) => { ev.stopPropagation(); openEdit(item) }}
                                className="px-3 py-1.5 bg-yellow-50 text-xs font-medium rounded-lg text-yellow-700 hover:bg-yellow-100 flex items-center gap-1"
                            >
                              <Edit2 size={12} /> Edit
                            </button>
                            <button
                                onClick={(ev) => { ev.stopPropagation(); handleDelete(item) }}
                                className="px-3 py-1.5 bg-red-50 text-xs font-medium rounded-lg text-red-700 hover:bg-red-100 flex items-center gap-1"
                            >
                      ...
                      <div className="p-6 md:p-8 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-gray-500 text-sm mt-1">
                            Configure product SKUs, pricing and taxonomy.
                          </p>
                        </div>
                        <button
                            onClick={closeModal}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                          ...
                          <div>
                            ...
                            <input
                                {...register('code')}
                                className={cn(
                                    "w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 bg-gray-50 font-mono tracking-tight",
                                    errors.code ? "border-red-500" : "border-gray-200 focus:border-primary"
                                )}
                                placeholder="MAC-001"
                            />
                            {errors.code && (
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <input
                                  ...
                                  className={cn(
                                      "w-full pl-9 pr-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20",
                                      errors.unit_price ? "border-red-500" : "border-gray-200 focus:border-primary"
                                  )}
                                  placeholder="0.00"
                              />
                            </div>
