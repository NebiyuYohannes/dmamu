...
import {
  ...
  Edit2, Trash2, MapPin, Archive, Plus
} from 'lucide-react'
import {
...
export default function Inventory() {
  ...
  const { data, isLoading: loading } = useQuery({
    ...
    queryFn: () => getWarehouses({ search: debouncedSearch, ordering, page }),
    placeholderData: (prev) => prev,
    staleTime: 60 * 1000,
  })

  // Service already unwraps .data — these are ready to use
  ...
  const saveMutator = useMutation({
    ...
    onMutate: async (newWarehouse) => {
      ...
      const previous = queryClient.getQueryData(
          ['warehouses', page, debouncedSearch, ordering]
      )
  ...
  function handleDelete(item) {
    if (window.confirm(
        'If you delete this warehouse you will lose all stock products inside it. Proceed?'
    )) {
          ...
          <main className="flex-1 p-6 md:p-8 md:ml-64 w-full transition-all duration-300">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-500 text-sm mt-2">
                Track stock levels, manage products, and monitor inventory movements across your warehouse.
              </p>
            </div>

            <div className="mb-6"><InventoryTab /></div>
                      ...
                      rawItems.map((item) => (
                          ...
                          >
                            <td className="py-3 px-4">
                              <a
                                  ...
                                  className="cursor-pointer font-medium text-gray-900 hover:text-primary transition-colors"
                              >
                                {item.name}
                              </a>
                            </td>
                          ...
                          <span className="text-sm text-gray-900 font-medium">
                            ...
                            <td className="py-3 px-4">
                              <p className={cn(
                                  "text-sm font-medium",
                                  String(item.total_worth || '').includes('$')
                                      ? (item.total_worth === '$0.00' ? 'text-gray-900' : 'text-green-600')
                                      : 'text-gray-900'
                              )}>
                ...
                return (
                    <div className="flex items-center justify-between p-4 border-t border-gray-200">
                      ...
                      <div className="flex gap-2 w-full sm:w-auto justify-between sm:justify-end">
                        ...
                        <span className="text-sm text-gray-500 sm:hidden self-center">
                      Page {page}
                    </span>
                        <button
